...
//                    GLES20.GL_CLAMP_TO_EDGE);
    ...
    public static int loadProgram(String vSource, String fSource) {
         ...
         * 顶点着色器
         */
        Log.d(TAG, "loadProgram: 1");
        int vShader = GLES20.glCreateShader(GLES20.GL_VERTEX_SHADER);
        Log.d(TAG, "loadProgram: 2");
        //加载着色器代码
        GLES20.glShaderSource(vShader, vSource);
        Log.d(TAG, "loadProgram: 3");
        //编译（配置）
        GLES20.glCompileShader(vShader);
        Log.d(TAG, "loadProgram: 4");

        //查看配置 是否成功
        int[] status = new int[1];
        GLES20.glGetShaderiv(vShader, GLES20.GL_COMPILE_STATUS, status, 0);
        Log.d(TAG, "loadProgram: 5 ===》status[0]="+status[0]);
        if (status[0] != GLES20.GL_TRUE) {
            Log.d(TAG, "loadProgram: ==》"+status[0]);
            //失败
            throw new IllegalStateException("load vertex shader:" + GLES20.glGetShaderInfoLog
                    (vShader));
        }

        Log.d(TAG, "loadProgram: 6");
        ...
        Log.d(TAG, "loadProgram: 7");
        //加载着色器代码
        GLES20.glShaderSource(fShader, fSource);
        Log.d(TAG, "loadProgram: 8");
        //编译（配置）
        GLES20.glCompileShader(fShader);

        Log.d(TAG, "loadProgram: 9");
        //查看配置 是否成功
        GLES20.glGetShaderiv(fShader, GLES20.GL_COMPILE_STATUS, status, 0);
        Log.d(TAG, "loadProgram: 10 ===》status[0]="+status[0]);
        if (status[0] != GLES20.GL_TRUE) {
            //失败
            Log.d(TAG, "loadProgram: fShader error"+ GLES20.glGetShaderInfoLog(fShader));
            throw new IllegalStateException("load fragment shader:" + GLES20.glGetShaderInfoLog
                    (fShader));
        }

        Log.d(TAG, "loadProgram: 11");
        /**
         * 创建着色器程序
         */
